"""
TTF字体解析器 - 使用freetype-py
TTF Font Parser - Using freetype-py

使用freetype库解析TTF字体，将字符渲染为像素缓冲区
"""
...
class TTFParser:
    ...
    def __init__(self, ttf_path, font_size, config=None):
        ...
        try:
            ...
            raise Exception(f"无法加载TTF字体文件: {ttf_path}\n错误信息: {e}")
...
def main():
    ...
    # 配置参数
    # ttf_file = r"../font/04b_03/04B_03__.TTF"
    ttf_file = r"../font/ZlabsBitmap_12px_ttf/ZlabsBitmap_12px_CN.ttf"
    font_size = 12
    text_to_render = preview_config.get('text', 'Hello WouoUI')
    
    print(f"字体文件: {ttf_file}")
    ...
    # 创建解析器
    try:
        parser = TTFParser(ttf_file, font_size)
        print(f"[OK] 字体加载成功")
