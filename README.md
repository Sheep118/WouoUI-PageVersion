...
- `CallBackFunc` : 回调函数类型，每个页面都需要一个回调函数(如果没有回调函数，在对应初始化函数中置NULL即可)。
  ...
  需要注意的是每个页面都有成员`Page.auto_deal_with_msg`，用于控制页面是否会自动处理Msg，默认是使能的，页面会根据输入的Msg自动进行UI的操作，像切换选项等(**这里有一点需要特别注意的是，在弹窗页面的默认处理中，除了SpinWin外其他弹窗默认click消息会拥有和return一样的效果，即点击设置后会返回，此时如果想要click跳转其他页面的话，就需要通过下面这个接口函数失能消息自动处理，并在回调函数中调用提供的api函数完成对6种(上/下/左/右/click/return)消息的处理**)，具体可以看看例子中PC_Simulate中listwin嵌套val_win的代码。
