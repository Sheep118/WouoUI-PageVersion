...
typedef enum {
    type_none =
        0x00, // 非页面类型,用于在jump函数判断一个页面是否经过初始化,如果没有,则没有绑定过方法,避免状态机访问方法报错
    type_title,       // 磁贴类
    type_list,        // 列表类
    type_wave,        // 波形类
...
// 更改回调函数直接传入当前页面的页面地址
typedef bool (*CallBackFunc)(
    const Page* cur_page_addr,
    InputMsg msg); // 页面回调函数类型定义(会传入当前页面的地址和当前页面收到的操作消息)
// 页面地址类型，用于声明全局页面数组存放页面使用
#define PageAddr void*
// 每个页面基本方法的定义
typedef bool (*PageIn)(
    PageAddr); // 进入页面的过度动画函数(返回值为True表示这个状态已经可以结束了，切换下一个状态)
typedef void (*PageInParaInit)(PageAddr); // 进入页面过度动画参数初始化函数
typedef void (*PageShow)(PageAddr);       // 页面的展示函数
typedef bool (*PageReact)(
    PageAddr); // 页面的响应函数(返回值为True表示这个状态已经可以结束了，切换下一个状态)
...
struct _page {
    ...
    bool
        auto_deal_with_msg; // 页面是否会自动处理消息,默认会,如果置False,需要自己在回调函数中调用提供的接口进行页面操作
...
#endif
#define TILE_ASSERT_IND_H                                                                          \
    (TILE_ICON_IND_R + TILE_ICON_IND_L > DEFAULT_TILE_ICON_S) // 检查指示器水平大小
...
#define CHECK_BOX_D_H                                                                              \
    CHECK_BOX_F_H - 2 * (CHECK_BOX_D_S + 1)      // 选择框里面的点高度(这里加的常量1是外框的线条宽)
...
#define LIST_LINETAIL_SPIN_PREFIX                                                                  \
    "%$" // listtext中使用这些字符其中一个，行尾会把val按照decimal显示为定点数
#define LIST_LINETAIL_CONF_PREFIX                                                                  \
    "@#" // listtext中使用这些字符其中一个，行尾会显示一个单选框(其中第一个作为二值选项框会自动处理,如果开启自动处理的话)
#if (LIST_VAL_MAX_LEN <                                                                            \
     6) // Val的空间是由Text长度宏计算来的，因此如果这个长度不足最小字符宽度6的话，报一个warning
#    warning                                                                                       \
        "LIST_VAL_MAX_LEN is too small, the reason may be that DEFAULT_LIST_TEXT_LEN_PER_MAX is too large OR the WOUOUI_BUFF_WIDTH is too small"
#endif
#define LIST_MACRO_ASSERT                                                                          \
    ((CHECK_BOX_R * 2 > CHECK_BOX_F_W) || (CHECK_BOX_D_R * 2 > CHECK_BOX_D_W) ||                   \
     (LIST_VAL_MAX_LEN < CHECK_BOX_F_W))
// 检查check
// box的倒角是否符合大小(倒角太大会自动取到能取的最大值的，但还是提示一下)，这个检查在Init中执行，返回True需要打印log提示
// List页面类型定义
 ...
 */
void WouoUI_ListPageInit(ListPage* lp, uint8_t item_num, Option* option_array,
                         PageSetting page_setting, CallBackFunc call_back);
/**
 * @brief 移动到列表的上一个项目
 * @param lp 列表页面指针
 * @attention 此函数只建议在回调函数中使用
...
#if (WAVE_DEPTH <                                                                                  \
     WAVE_SHOW_WIDTH +                                                                             \
         1) // 波形深度必须于宽度+1,不然显示上有缺失，因为深度是数组的大小，报一个warning
#    warning                                                                                       \
        "WAVE_DEPTH is too small, the wave will be lost some point, not zoom to fill the screen"
#endif
// 类型定义
// 波形页面的类变量
struct WavePageVar {
 ...
 * @param call_back 回调函数
 */
void WouoUI_WavePageInit(WavePage* wp, uint8_t wave_num, WaveData* wave_data_array,
