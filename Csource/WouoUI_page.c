...
void WouoUI_SetPageAutoDealWithMsg(Page* page, bool open) {
    if (WouoUI_CheckPageIsInit(page)) {
        ...
        WOUOUI_LOG_E("this page is not inited!!,it can't disable/enable auto dealwth msg");
}
...
bool WouoUI_TitlePageIn(PageAddr page_addr) {
    ...
    // 计算动画参数
    WouoUI_Animation(&p_cur_ui->tp_var.iconX, p_cur_ui->upara->ani_param[TILE_ANI], p_cur_ui->time,
                     &(p_cur_ui->anim_is_finish)); // 图标x坐标
    WouoUI_Animation(&p_cur_ui->tp_var.iconY, p_cur_ui->upara->ani_param[TILE_ANI], p_cur_ui->time,
                     &(p_cur_ui->anim_is_finish)); // 图标y坐标
    WouoUI_Animation(&p_cur_ui->tp_var.barX, p_cur_ui->upara->ani_param[TILE_ANI], p_cur_ui->time,
                     &(p_cur_ui->anim_is_finish)); // 装饰条x坐标
    WouoUI_Animation(&p_cur_ui->tp_var.titleY, p_cur_ui->upara->ani_param[TAG_ANI], p_cur_ui->time,
    ...
    for (uint8_t i = 0; i < tp->item_num; i++) {
        if (p_cur_ui->upara->ufd_param[TILE_UFD]) // 从第一个选项开始展开，最终保持选中在中间
            temp = ((WOUOUI_BUFF_WIDTH - TILE_ICON_W) >> 1) + i * p_cur_ui->tp_var.iconX.pos_cur -
                   TILE_ICON_S * tp->select_item;
    ...
    if (p_cur_ui->tp_var.iconX.pos_cur == p_cur_ui->tp_var.iconX.pos_tgt) {
        ret = true;
        p_cur_ui->tp_var.iconX.pos_cur = p_cur_ui->tp_var.iconX.pos_tgt =
            -1 * tp->select_item * TILE_ICON_S;
    }
    return ret;
}
...
void WouoUI_TitlePageIndicatorCtrl(PageAddr page_addr) {
    ...
    WouoUI_CanvasDrawBoxRightAngle(&(p_cur_ui->w_all), p_cur_ui->indicator.x.pos_cur,
                                   ...
                                   p_cur_ui->indicator.h.pos_cur, TILE_ICON_IND_SL);
...
bool WouoUI_TitlePageReact(PageAddr page_addr) {
    ...
    if (tp->page.auto_deal_with_msg) { // 使能自动处理msg的话
        ...
        if (msg != msg_none && NULL != tp->page.cb) // 任何输入的有效信息都会调用回调函数
            tp->page.cb(
                &(tp->page),
                msg); // 自动处理时,会忽略回调函数返回值(title内只有msg_return输入时才会返回)
    } else {
    ...
    CallBackFunc call_back) // 回调函数，参数为确认选中项index（1-256）0表示未确认哪个选项
{
    if (TILE_MACRO_ASSERT)
        WOUOUI_LOG_W("the size of fnot large than the hight of bar in TitlePage");
    ...
    for (uint8_t i = 0; i < title_page->item_num; i++) {
        ...
        if (NULL == title_page->option_array[i].text) {
            ...
            title_page->option_array[i].text =
                (char*)WOUOUI_WIN_TXT_DEFAULT; // 使用默认文本提示noset
            WOUOUI_LOG_E("The text of %d th item in TitlePgae NO set!!!", i);
        }
    }
    WouoUI_TitlePageInParaInit(
        (PageAddr)title_page); // 做一次进入参数初始化，因为默认UI中没有设置类变量的值
}
// Title提供的接口函数
...
static void WouoUI_ListAuotCanvasDrawLineTailValTxt(
    ...
    if (txt_str_len + LIST_TEXT_L_S * 2 + LIST_TEXT_R_S + val_str_len + LIST_IND_VAL_S <
        ...
        p_canvas_val->start_x =
            WOUOUI_BUFF_WIDTH - val_str_len - LIST_TEXT_R_S; // 放得下的时候值靠右侧放
...
static void WouoUI_ListDrawText_CheckBox(int16_t start_y, Option* item, uint8_t select_order) {
    ...
    if (NULL != strchr(LIST_LINETAIL_VAL_PREFIX, item->text[0])) {
        ...
        WouoUI_ListAuotCanvasDrawLineTailValTxt(item, &canvas_txt, &canvas_val, val_buff,
                                                select_order);
        ...
        WouoUI_CanvasDrawRBoxEmpty(
            &(p_cur_ui->w_all), WOUOUI_BUFF_WIDTH - CHECK_BOX_R_S - CHECK_BOX_F_W,
            start_y + CHECK_BOX_U_S, CHECK_BOX_F_W, CHECK_BOX_F_H, CHECK_BOX_R);
        if (item->val != 0) // 非0即是选中，画框内的点(这里加的常量1是外框的线条宽)
            WouoUI_CanvasDrawRBox(&(p_cur_ui->w_all),
                                  WOUOUI_BUFF_WIDTH - CHECK_BOX_R_S - CHECK_BOX_F_W +
                                      CHECK_BOX_D_S + 1,
                                  start_y + CHECK_BOX_U_S + CHECK_BOX_D_S + 1, CHECK_BOX_D_W,
                                  CHECK_BOX_D_H, CHECK_BOX_D_R);
    } else if (NULL != strchr(LIST_LINETAIL_TXT_PREFIX, item->text[0])) { // 如果是列表弹窗
        if (NULL != item->content) {
            WouoUI_ListAuotCanvasDrawLineTailValTxt(item, &canvas_txt, &canvas_val, item->content,
                                                    select_order);
        }
    } else
        canvas_txt.w = MIN(WouoUI_GetStrWidth(item->text, LIST_TEXT_FONT),
    ...
    if (item->order == select_order) {
        ...
        p_cur_ui->lp_var.indicator_w_temp =
            LIST_TEXT_L_S * 2 + canvas_txt.w; // 计算出每一项的宽度后，顺便给inductor的宽度赋值
...
bool WouoUI_ListPageIn(PageAddr page_addr) {
    ...
    for (uint8_t i = 0; i < (lp->item_num); i++) {
        if (p_cur_ui->upara->ufd_param[LIST_UFD]) // 从头展开
            item_y = i * (p_cur_ui->lp_var.optInt.pos_cur) - list_line_h * (lp->select_item) +
                     lp->ind_y_tgt;
        else // 选中项展开
    ...
    if (p_cur_ui->lp_var.optInt.pos_cur == p_cur_ui->lp_var.optInt.pos_tgt) { // 动画抵达目标位置
        ret = true;
        p_cur_ui->lp_var.optInt.pos_cur = p_cur_ui->lp_var.optInt.pos_tgt =
            -list_line_h * (lp->select_item) + lp->ind_y_tgt;
...
void WouoUI_ListPageIndicatorCtrl(PageAddr page_addr) {
    ...
    WouoUI_CanvasDrawRBox(&(p_cur_ui->w_all), p_cur_ui->indicator.x.pos_cur,
                          p_cur_ui->indicator.y.pos_cur, p_cur_ui->indicator.w.pos_cur,
                          p_cur_ui->indicator.h.pos_cur, LIST_IND_BOX_R);
    WouoUI_GraphSetPenColor(1); // 恢复实色绘制
}
...
void WouoUI_ListPageScrollBarCtrl(PageAddr page_addr) {
    ...
    p_cur_ui->scrollBar.y.pos_tgt =
        (int16_t)(lp->select_item * WOUOUI_BUFF_HEIGHT / (lp->item_num - 1)) + 1;
    WouoUI_CanvasDrawLine_H(&(p_cur_ui->w_all), WOUOUI_BUFF_WIDTH - LIST_BAR_W, WOUOUI_BUFF_WIDTH,
                            0);
    WouoUI_CanvasDrawLine_H(&(p_cur_ui->w_all), WOUOUI_BUFF_WIDTH - LIST_BAR_W, WOUOUI_BUFF_WIDTH,
                            WOUOUI_BUFF_HEIGHT - 1);
    WouoUI_CanvasDrawLine_V(&(p_cur_ui->w_all), WOUOUI_BUFF_WIDTH - ((LIST_BAR_W / 2) + 1), 0,
                            WOUOUI_BUFF_HEIGHT);
    WouoUI_CanvasDrawRBox(&(p_cur_ui->w_all), WOUOUI_BUFF_WIDTH - LIST_BAR_W, 0, LIST_BAR_W,
    ...
    if (lp->page.auto_deal_with_msg) {
        ...
        case msg_click: // 如果没有使用自动处理msg的话,二值框的值和是否是单选项肯定也是用户自己处理嘞
            ...
            if (selcet_string[0] == ((char*)LIST_LINETAIL_CONF_PREFIX)[0]) { // 二值选框
                lp->option_array[lp->select_item].val =
                    !lp->option_array[lp->select_item].val; // 将对应值取反
            }
            // 单选框页面
...
void WouoUI_ListPageInit(
    ...
    if (LIST_MACRO_ASSERT)
        WOUOUI_LOG_W("The chamfer of the check box is too large, exceeding its width.");
...
Option* WouoUI_ListTitlePageGetSelectOpt(const Page* cur_page_addr) {
    ...
    case type_title: // 如果是title页面
        opt =
            &(((TitlePage*)cur_page_addr)->option_array[((TitlePage*)cur_page_addr)->select_item]);
        break;
...
void WouoUI_ListPageLastItem(ListPage* lp) { // 重置动画参数
    WouoUI_CanvasSlideStrReset(&(p_cur_ui->lp_var.opt_text_ss));
    WouoUI_CanvasSlideStrReset(&(p_cur_ui->lp_var.opt_val_ss));
    const uint8_t list_line_h = LIST_LINE_H; // 因为行高是用宏计算的，这里用变量保存不用每次都计算
    if (lp->select_item == 0) {                       // 选中第一个的话
        if (p_cur_ui->upara->loop_param[LIST_LOOP]) { // 同时loop参数开的话，从顶部滑动到底部
            ...
            if (lp->item_num > lp->line_n) {          // 数目超出一页的最大数目
                ...
                p_cur_ui->lp_var.optInt.pos_tgt =
                    WOUOUI_BUFF_HEIGHT - (lp->item_num) * list_line_h; // 更改文字到最底
            } else                                                     // 没有超出数目则是到最后一个
        ...
        lp->select_item--; // 选中减1
        if ((lp->select_item - 1) <
            -((p_cur_ui->lp_var.optInt.pos_tgt) / list_line_h)) { // 光标盒子到页面顶了
            if (!(WOUOUI_BUFF_HEIGHT % list_line_h))              // 上面剩余完整的行
                ...
                if (lp->ind_y_tgt ==
                    WOUOUI_BUFF_HEIGHT -
                        list_line_h *
                            lp->line_n) { // 文字往下走一行，且光标盒子置于0处，把上面的非整行去掉
                    p_cur_ui->lp_var.optInt.pos_tgt +=
                        (lp->line_n + 1) * list_line_h - WOUOUI_BUFF_HEIGHT;
                    lp->ind_y_tgt = 0;
...
void WouoUI_ListPageNextItem(ListPage* lp) { // 重置动画参数
    WouoUI_CanvasSlideStrReset(&(p_cur_ui->lp_var.opt_text_ss));
    WouoUI_CanvasSlideStrReset(&(p_cur_ui->lp_var.opt_val_ss));
    const uint8_t list_line_h = LIST_LINE_H; // 因为行高是用宏计算的，这里用变量保存不用每次都计算
    if (lp->select_item == (lp->item_num) - 1) {      // 到最后一个选项了
        ...
        lp->select_item++;
        if ((lp->select_item + 1) >
            ((lp->line_n) - (p_cur_ui->lp_var.optInt.pos_tgt) / list_line_h)) { // 光标到页面底
            if (!(WOUOUI_BUFF_HEIGHT % list_line_h))                            // 上面剩余完整的行
...
static bool _WouoUI_indexIsInShowRange(uint16_t idx, uint16_t idx_show_head,
                                       uint16_t idx_show_tail) {
...
static void _WouoUI_WaveUpdateRange(WaveData* wd) {
    ...
    if (!_WouoUI_indexIsInShowRange(wd->idx_show_max, idx_show_head,
                                    idx_show_tail)) { // 如果最大值已经移出展示范围
        int16_t max =
            -32768; // 从新计算一次最大值(用最小值作为比较初值，省去了第一次判断，还是很巧妙的)
    ...
    if (!_WouoUI_indexIsInShowRange(wd->idx_show_min, idx_show_head,
                                    idx_show_tail)) { // 如果最小值已经移出展示范围
    ...
    if (wd->data[idx_show_head] >=
        wd->data[wd->idx_show_max]) {     // 新加入的数据大于最大值，更新最大值
        wd->idx_show_max = idx_show_head; // 新加入的数据大于最大值，更新最大值
        maxFlag = true;
    }
    if (wd->data[idx_show_head] <=
        wd->data[wd->idx_show_min]) { // 新加入的数据小于最大值，更新最大值
        wd->idx_show_min = idx_show_head;
        minFlag = true;
    }
    if (wd->data[(idx_show_tail + WAVE_DEPTH - 1) % WAVE_DEPTH] >=
        wd->data[wd->idx_show_max]) { // 新加入的数据大于最大值，更新最大值
...
void WouoUI_WavePageShow(PageAddr page_addr) {
    ...
    for (uint16_t j = wd->idx_show_head; j != wd->idx_show_tail; j++, j %= WAVE_DEPTH) {
        ...
        p_cur_ui->wt_var.cur_y =
            WAVE_BOX_H + WAVE_BOX_U_S -
            (data_val - wd->rangeMin) * WAVE_BOX_H / (wd->rangeMax - wd->rangeMin) - 1;
        if (j == wd->idx_show_head)
            p_cur_ui->wt_var.last_y = p_cur_ui->wt_var.cur_y;
        // 绘制波形
        switch (wd->waveType) {
        case WaveType_Solid: // 实线
            if (p_cur_ui->wt_var.cur_y > (p_cur_ui->wt_var.last_y + 1))
                WouoUI_CanvasDrawLine_V(&(p_cur_ui->w_all), WAVE_BOX_L_S + wave_x,
                                        p_cur_ui->wt_var.last_y + 1, p_cur_ui->wt_var.cur_y);
            else if (p_cur_ui->wt_var.cur_y < (p_cur_ui->wt_var.last_y - 1))
                WouoUI_CanvasDrawLine_V(&(p_cur_ui->w_all), WAVE_BOX_L_S + wave_x,
                                        p_cur_ui->wt_var.last_y - 1, p_cur_ui->wt_var.cur_y);
            else
                WouoUI_CanvasDrawPoint(&(p_cur_ui->w_all), WAVE_BOX_L_S + wave_x,
                                       p_cur_ui->wt_var.cur_y);
            break;
        ...
        case WaveType_Fill: // 填充波形
            WouoUI_CanvasDrawLine_V(&(p_cur_ui->w_all), WAVE_BOX_L_S + wave_x,
                                    WAVE_BOX_U_S + WAVE_BOX_H - 1, p_cur_ui->wt_var.cur_y);
            break;
        default:
            break;
        }
    ...
    if (false == wd->stop_flag)
        WouoUI_CanvasDrawRBox(&(p_cur_ui->w_all), 1, WAVE_BOX_U_S + WAVE_BOX_H + WAVE_BOX_D_S,
                              WAVE_STOP_SIG_HW, WAVE_STOP_SIG_HW,
                              UINT_DIVISION_CELL(WAVE_STOP_SIG_HW, 3));
    else
        WouoUI_CanvasDrawRBox(&(p_cur_ui->w_all), 1, WAVE_BOX_U_S + WAVE_BOX_H + WAVE_BOX_D_S,
                              WAVE_STOP_SIG_HW, WAVE_STOP_SIG_HW, 0);
    WouoUI_Animation(&p_cur_ui->wt_var.text_y, p_cur_ui->upara->ani_param[WAVE_ANI], p_cur_ui->time,
                     &(p_cur_ui->anim_is_finish)); // 文字y坐标
    WouoUI_Animation(&p_cur_ui->wt_var.y_axis_val_x, p_cur_ui->upara->ani_param[WAVE_ANI],
                     p_cur_ui->time, &(p_cur_ui->anim_is_finish)); // 文字x坐标
    // 绘制Title字符串
    ...
    if (p_cur_ui->anim_is_finish) { // anim结束，且单次滚动没有完成就使能滚动
        ...
        if (!(p_cur_ui->wt_var.val_ss.slide_is_finish))
            p_cur_ui->wt_var.val_ss.slide_enable = true;
        if (!(p_cur_ui->wt_var.max_val_ss.slide_is_finish))
            p_cur_ui->wt_var.max_val_ss.slide_enable = true;
        if (!(p_cur_ui->wt_var.mid_val_ss.slide_is_finish))
            p_cur_ui->wt_var.mid_val_ss.slide_enable = true;
        if (!(p_cur_ui->wt_var.min_val_ss.slide_is_finish))
            p_cur_ui->wt_var.min_val_ss.slide_enable = true;
    }
    // 纵轴刻度
    ...
    WouoUI_CanvasDrawPoint(&(p_cur_ui->w_all), axis_tick_x,
                           p_cur_ui->indicator.y.pos_cur +
                               UINT_DIVISION_ROUND(p_cur_ui->indicator.h.pos_cur, 2) - 1);
    WouoUI_CanvasDrawPoint(&(p_cur_ui->w_all), axis_tick_x,
                           p_cur_ui->indicator.y.pos_cur + p_cur_ui->indicator.h.pos_cur - 1);
    p_cur_ui->slide_is_finish =
        !(p_cur_ui->wt_var.title_ss.slide_enable) && !(p_cur_ui->wt_var.val_ss.slide_enable) &&
...
bool WouoUI_WavePageReact(PageAddr page_addr) {
    ...
    if (wp->page.auto_deal_with_msg) { // 自动处理msg的话
        ...
        if (msg_none != msg && NULL != wp->page.cb)
...
void WouoUI_WavePageIndicatorCtrl(PageAddr page_addr) {
    ...
    WouoUI_CanvasDrawRBoxEmpty(&(p_cur_ui->w_all), p_cur_ui->indicator.x.pos_cur,
                               p_cur_ui->indicator.y.pos_cur, p_cur_ui->indicator.w.pos_cur,
                               p_cur_ui->indicator.h.pos_cur, 0);
}

void WouoUI_WavePageInit(WavePage* wp,              // 波形页面对象
...
void WouoUI_WavePageUpdateVal(WavePage* wp, uint8_t wave_num, int16_t new_data) { // 更新波形数据
    if (wave_num >= wp->wave_data_num) {
        WOUOUI_LOG_E("Valid wave_num, it is too large");
        return;
    }
    ...
    if (false == wave_data->stop_flag) // 波形运动时才能更新数据
    {
        ...
        if (((wave_data->idx_show_tail + WAVE_DEPTH - wave_data->idx_show_head) % WAVE_DEPTH) >
            WAVE_SHOW_WIDTH) { // 如果显示范围扩大了
            wave_data->idx_show_head++;
            wave_data->idx_show_head %= (WAVE_DEPTH); // 整体后移
        }
        if (wave_data->idx_tail ==
            wave_data->idx_head) { // 队列满了,直接覆盖(注意尾指针已经后移过了)
            wave_data->idx_head++;
...
void WouoUI_WavePageStopRestartWave(WavePage* wp, uint8_t wave_num, bool stop) {
    ...
    if (false == wd->stop_flag) { // 从true->false,停止后从新开始，需要把展示指针移到最新的数据处
        ...
        wd->idx_show_min =
            wd->idx_show_tail; // 把min和max的指针强制置到show的范围外(因为上面向右移动了不止一个点)
        _WouoUI_WaveUpdateRange(wd); // 这样调用这个函数可以强制对所有数据进行一次轮询最大最小值
    }
}
void WouoUI_WavePageShowLastWaveData(WavePage* wp) {
    _WouoUI_WaveAnimParaReaset();
    if (wp->wave_data_select == 0)
        wp->wave_data_select = wp->wave_data_num - 1; // 选中最后一个
    else
        wp->wave_data_select--; // 选中减1
}
void WouoUI_WavePageShowNextWaveData(WavePage* wp) {
    _WouoUI_WaveAnimParaReaset();
    if (wp->wave_data_select == wp->wave_data_num - 1)
        wp->wave_data_select = 0; // 选中第一个
    else
        wp->wave_data_select++; // 选中加1
}
bool WouoUI_WavePageLeftShiftWave(
    WavePage* wp, uint8_t wave_num) // 返回值表示是否已经到头了(即有没有成功shift波形)
{
    bool ret = WouoUI_WavePageCanShiftWave(wp, wave_num, 0); // 能否左移
    if (ret) {
        WaveData* wd = &(wp->wave_data_array[wave_num]);
        if (wd->idx_show_head == 0)
            wd->idx_show_head = WAVE_DEPTH - 1;
        else
            wd->idx_show_head--; // 整体前移
        if (wd->idx_show_tail == 0)
            wd->idx_show_tail = WAVE_DEPTH - 1;
        else
            wd->idx_show_tail--;     // 整体前移
...
bool WouoUI_WavePageRightShiftWave(WavePage* wp, uint8_t wave_num) {
    ...
    if (ret) {
        WaveData* wd = &(wp->wave_data_array[wave_num]);
        wd->idx_show_head++;
        wd->idx_show_head %= (WAVE_DEPTH); // 整体后移
        wd->idx_show_tail++;
        wd->idx_show_tail %= (WAVE_DEPTH); // 整体后移
        _WouoUI_WaveUpdateRange(wd);       // 更新数据范围
        ret = true;                        // 成功移动波形
    }
    return ret;
...
bool WouoUI_WavePageCanShiftWave(WavePage* wp, uint8_t wave_num,
                                 uint8_t left0ORright1) // 返回true,表示能够移动
{
    bool ret = false;
    if (wave_num >= wp->wave_data_num) {
        WOUOUI_LOG_E("Valid wave_num, it is too large");
    ...
    if (wd->stop_flag) { // 只有波形停止的时候才能移动波形
        if (left0ORright1 == 0)
            ret = ((wd->idx_show_head == wd->idx_head) ? false : true); // 能够左移
        else
            ret = ((wd->idx_show_tail == wd->idx_tail) ? false : true); // 能够右移
    } else {
        WOUOUI_LOG_I("Wave data is run, Can't shift it");
    }
    return ret;
}
