 ...
 */
void WouoUI_MsgWinPageInit(MsgWin* mw, char* content, bool auto_get_bg_opt, uint16_t move_step,
                           CallBackFunc cb);
/**
 * @brief 设置消息窗口的显示内容
 *
...
typedef struct {
    ...
    uint16_t str_move_step; // 字符串移动时的步长
    bool conf_ret
        : 1; // 确认弹窗的结果，如果使能自动获取的话，这个成员和content的值都会自动被置成lastPage的选中项
    bool auto_get_bg_opt : 1; // 弹窗页面是否自动获得lastPage中的select_opt
 ...
 * @param move_step 移动步长
 * @param cb 回调函数
 * @attention auto_get_bg_opt
 * 为true,会自动获取背景页面选中项的val和content覆盖conf_ret和content。auto_set_bg_opt同理
 */
void WouoUI_ConfWinPageInit(ConfWin* cw, char* content, char* str_left, char* str_right,
                            bool conf_ret, bool auto_get_bg_opt, bool auto_set_bg_opt,
                            uint16_t move_step, CallBackFunc cb);
/**
 * @brief 向上滑动配置窗口页面文本
 *
 * @param cw 配置窗口对象指针
 ...
 * @param auto_set_bg_opt 自动设置背景选项
 * @param cb 回调函数
 * @attention auto_get_bg_opt 为true会自动获取背景页面选中项的text作为文本，获取val作为init_val,
 * auto_set_bg_opt同理
 */
void WouoUI_ValWinPageInit(ValWin* vw, char* text, int32_t init_val, int32_t min, int32_t max,
                           int32_t step, bool auto_get_bg_opt, bool auto_set_bg_opt,
                           CallBackFunc cb);
/**
...
#endif
#define SPIN_WIN_MACRO_ASSERT                                                                      \
    (SPIN_WIN_H > WOUOUI_BUFF_HEIGHT ||                                                            \
     (9 * GET_FNOT_W(SPIN_WIN_NUM_FONT) + 6 * (SPIN_WIN_NUM_S) > SPIN_WIN_W))
// 类型定义
struct SpinWinVar {
    AnimPos win_y;      // 弹窗的竖直动画
    SlideStr text_ss;   // 文本的滚动动画
 ...
 * @param cb 回调函数
 * @attention auto_get_bg_opt 为true会自动获取背景页面选中项的text作为文本，获取val作为init_val,
 * 获取opt的小数位数作为dec_num, auto_set_bg_opt同理
 */
void WouoUI_SpinWinPageInit(SpinWin* spw, char* text, int32_t init_val, DecimalNum dec_num,
                            int32_t min, int32_t max, bool auto_get_bg_opt, bool auto_set_bg_opt,
                            CallBackFunc cb);
/**
 * @brief 移动数值选择窗口的选择位
 * @param spw 指向SpinWin结构体的指针
 ...
 * @param cb 回调函数指针
 * @attention auto_set_bg_opt
 * 为true会自动将选中项的内容设置到背景页面选中选项的content中，注意，数组大小需要正确传入
 */
void WouoUI_ListWinPageInit(ListWin* lw, uint8_t array_num, String* str_array, bool auto_set_bg_opt,
                            CallBackFunc cb);
