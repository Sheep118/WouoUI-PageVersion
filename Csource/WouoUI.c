...
void WouoUI_FuncDoNothing(PageAddr page_addr) {
    UNUSED_PARAMETER(page_addr);
}
bool WouoUI_FuncDoNothingRetTrue(PageAddr page_addr) {
    UNUSED_PARAMETER(page_addr);
    return true;
}
bool WouoUI_FuncDoNothingRetFalse(PageAddr page_addr) {
    UNUSED_PARAMETER(page_addr);
    return false;
}
...
WouoUI default_ui = {
    ...
    .current_page = NULL,
    .pen = {.color_mode = PEN_MODE_NORMAL,
            .color = PEN_COLOR_BLACK,
            .rev_color_flag = 0}, // 全局画笔变量清零
            ...
            .title_ss =
                {
                    ...
                    .canvas =
                        {
                            .w = WOUOUI_BUFF_WIDTH - TILE_BAR_W,
                        },
                },
        },
    .tp_mth =
    ...
    .lp_var =
        {
            ...
            .opt_text_ss =
                {
                    .slide_mode = (SlideStrMode)LIST_TXT_SLIDESTR_MODE,
                },
            .opt_val_ss =
                {
                    .slide_mode = (SlideStrMode)LIST_VAL_SLIDESTR_MODE,
                },
        },
    .lp_mth =
        {
...
bool WouoUI_BlurProc(uint16_t time) {
    if (false == p_cur_ui->ui_blur
                     .blur_end) { // fade完成就不用fade了，可以作为一个开关，要开启fade的时候置false
...
#define PAGE_USE_METHOD(page, method)                                                              \
    (((Page*)page)->methods->method(page)) // 使用这个宏调用方法，避免传错指针

void WouoUI_IndicatorProc(
    Page* p) { // 同样为了提高对消息的响应能力，tgt和cur的坐标设置需要在Animation函数调用前，
    // 这样可以保证在soft动态刷新时一有消息进入，就可以接力到anim_is_finish到false使状态机全速运行
    PAGE_USE_METHOD(p, indicator_ctrl);
    // indicator anim
    WouoUI_Animation(&p_cur_ui->indicator.x, p_cur_ui->upara->ani_param[IND_ANI], p_cur_ui->time,
                     &(p_cur_ui->anim_is_finish));
    WouoUI_Animation(&p_cur_ui->indicator.y, p_cur_ui->upara->ani_param[IND_ANI], p_cur_ui->time,
                     &(p_cur_ui->anim_is_finish));
    WouoUI_Animation(&p_cur_ui->indicator.w, p_cur_ui->upara->ani_param[IND_ANI], p_cur_ui->time,
                     &(p_cur_ui->anim_is_finish));
    WouoUI_Animation(&p_cur_ui->indicator.h, p_cur_ui->upara->ani_param[IND_ANI], p_cur_ui->time,
                     &(p_cur_ui->anim_is_finish));
}

void WouoUI_ScrollBarProc(
    Page* p) { // 同样为了提高对消息的响应能力，tgt和cur的坐标设置需要在Animation函数调用前，
    // 这样可以保证在soft动态刷新时一有消息进入，就可以接力到anim_is_finish到false使状态机全速运行
    if (WouoUI_CheckPageTypeisWin(p) &&
        WouoUI_FuncDoNothing ==
            p->methods
                ->scrollbar_ctrl) { // 如果当前页面是弹窗页面同时弹窗页面没有对scrollbar操作的话
        PAGE_USE_METHOD(p->last_page,
                        scrollbar_ctrl); // 做背景页面的bar函数(因为部分页面有bar但弹窗不带bar函数)
    }
    PAGE_USE_METHOD(p, scrollbar_ctrl);
    WouoUI_Animation(&p_cur_ui->scrollBar.y, p_cur_ui->upara->ani_param[LIST_ANI], p_cur_ui->time,
                     &(p_cur_ui->anim_is_finish));
}
...
#if SOFTWARE_DYNAMIC_REFRESH
    if (!WouoUI_MsgQueIsEmpty(&(p_cur_ui->msg_queue)))
        p_cur_ui->is_motionless = false; // 有消息输入时开启动画
    if (p_cur_ui->current_page != NULL && !(p_cur_ui->is_motionless)) {
#else
    if (p_cur_ui->current_page != NULL) {
#endif
        bool ret_in = false,
             ret_blur = false; // 由于短路原则不直接在if中使用两个函数(存放in和 blur函数的返回值)
        Page* p_all =
            (Page*)p_cur_ui
                ->current_page; // 把当前使用的与全局指示器和bar交互的页面是哪个(默认是当前页面)
        WouoUI_BuffClear();
        switch (p_cur_ui->state) {
        case ui_page_in: // 进入动画(全页面渐显，弹窗是同时做渐隐)
        {
            if (WouoUI_CheckPageTypeisWin(
                    p_cur_ui
                        ->in_page)) { // 将要跳转的页面如果是弹窗的话，渐隐背景的同时要做in动画，
            ...
            if (ret_in && ret_blur) {
                p_cur_ui->current_page = p_cur_ui->in_page; // 在page_proc过程中，in cur
                                                            // page保持一致
        ...
        case ui_page_proc: // 页面显示
            p_cur_ui->anim_is_finish =
                true; // 假设每一次show过程的动画都是完成的，如果有动画未完成就会被置成false.
            p_cur_ui->slide_is_finish =
                true; // 同样假设每一次show中的滚动动画都是完成的，如果有动画未完成就会被置成false.
            ...
            if (PAGE_USE_METHOD(p_cur_ui->current_page, react)) // 页面react(退出时返回true)
            {
                if (WouoUI_CheckPageTypeisWin(p_cur_ui->current_page))
                    WouoUI_BlurParaInit(
                        0, p_cur_ui->upara->ani_param[FADE_ANI]); // 弹窗退出是做渐显参数初始化
                else
                    WouoUI_BlurParaInit(4, p_cur_ui->upara->ani_param[FADE_ANI]); // 渐隐参数初始化
        ...
        case ui_page_out: // 页面退出函数(全页面是做渐隐，弹窗页面做渐显)
            if (WouoUI_CheckPageTypeisWin(p_cur_ui->current_page)) {
                ...
                ret_in = PAGE_USE_METHOD(
                    p_cur_ui->current_page,
                    in); // 这个是弹窗out动画函数，起始参数不一样而已，绘制与in完全一样，所以直接用in
...
// 软件刷新和硬件刷新的判断都必须放在所有动画绘制之后
        ...
        if (p_cur_ui->state == ui_page_in || p_cur_ui->state == ui_page_out)
            ...
            p_cur_ui->is_motionless =
                p_cur_ui->anim_is_finish && p_cur_ui->slide_is_finish; // anim和slide动画都完成了
#endif
#if HARDWARE_DYNAMIC_REFRESH
    ...
    if (terminate_page != NULL) {
        if (WouoUI_CheckPageIsInit(terminate_page)) { // 检查页面是否初始化过
            ...
            if (WouoUI_CheckPageTypeisWin(terminate_page)) { // 如果要跳转的页面是弹窗的话
                ...
                WouoUI_BlurParaInit(
                    p_cur_ui->win_bg_blur,
                    p_cur_ui->upara
                        ->ani_param[FADE_ANI]); // 渐隐参数初始化(是弹窗的话上一个页面只消失一半)
                p_cur_ui->state = ui_page_in; // 弹窗页面在in渐隐中同时做in动画
            } else {
                WouoUI_BlurParaInit(
                    4, p_cur_ui->upara
                           ->ani_param[FADE_ANI]); // 渐隐参数初始化(不是弹窗的话上一个页面完全消失)
